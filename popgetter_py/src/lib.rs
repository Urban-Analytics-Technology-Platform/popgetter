...
async fn _search()-> DataFrame{
    ...
    search_results.0.select(&["metric_id","human_readable_name", "metric_description", "metric_hxl_tag","geometry_level"]).unwrap()
}
...
fn get_data_request(obj: &Bound<'_, PyAny>) -> PyResult<DataRequestSpec> {
    Ok(DataRequestSpec{
        geometry: GeometrySpec{include_geoms:true, geometry_level:None},
        region: vec![],
        metrics: vec![MetricSpec::Metric(::popgetter::metadata::MetricId::Hxl(r"\#population\+adults".into()))],
        years: None,
    })
}
...
#[pyfunction]
fn get(#[pyo3(from_py_with = "get_data_request")] data_request: DataRequestSpec)->PyResult<PyDataFrame>{
...
#[pyfunction]
fn search(#[pyo3(from_py_with = "get_search")] search_query: SearchRequest) -> PyResult<PyDataFrame> {
   let rt = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()?;
